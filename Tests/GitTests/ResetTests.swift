...
final class ResetTests: XCTestCase {
  ...
  func testResetSoft() throws {
    ...
    try FileManager.default.withTemporaryDirectory { local in
      ...
      XCTAssertEqual(try repo.current.id.description, "b1d2dbab22a62771db0c040ccf396dbbfdcef052")
      ...
      XCTAssertEqual(try repo.current.id.description, "c8b08c2ed176eaaf7cea877f774319a27684870a")
      XCTAssertEqual(try repo.status.count, 0)

      try repo.reset(to: .commit(repo.previous), operation: .soft)
      XCTAssertEqual(try repo.current.id.description, "41c143541c9d917db83ce4e920084edbf2a4177e")
      XCTAssertEqual(try repo.status.count, 2)

      let addition = try XCTUnwrap(repo.status.first(where: { $0.status == .indexNew }))
      XCTAssertEqual(addition.headToIndex?.to?.path, "file.txt")

      let deletion = try XCTUnwrap(repo.status.first(where: { $0.status == .indexDeleted }))
  ...
  func testResetMixed() throws {
    ...
    try FileManager.default.withTemporaryDirectory { local in
      ...
      let addition = try XCTUnwrap(repo.status.first(where: { $0.status == .workingTreeNew }))
      XCTAssertEqual(addition.indexToWorkingDirectory?.to?.path, "file.txt")

      let deletion = try XCTUnwrap(repo.status.first(where: { $0.status == .workingTreeDeleted }))
