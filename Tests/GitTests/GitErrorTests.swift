...
final class GitErrorTests: XCTestCase {
  ...
  func testCatching() {
    ...
    XCTAssertEqual(
      GitError.catching { throw GitError(code: .bareRepository) }, GIT_EBAREREPO.rawValue)
    XCTAssertEqual(
      GitError.catching { throw GitError(code: .unbornBranch) }, GIT_EUNBORNBRANCH.rawValue)
    XCTAssertEqual(GitError.catching { throw GitError(code: .unmerged) }, GIT_EUNMERGED.rawValue)
    XCTAssertEqual(
      GitError.catching { throw GitError(code: .nonFastForward) }, GIT_ENONFASTFORWARD.rawValue)
    XCTAssertEqual(
      GitError.catching { throw GitError(code: .invalidSpec) }, GIT_EINVALIDSPEC.rawValue)
    ...
    XCTAssertEqual(
      GitError.catching { throw GitError(code: .certificate) }, GIT_ECERTIFICATE.rawValue)
    ...
    XCTAssertEqual(
      GitError.catching { throw GitError(code: .uncommitted) }, GIT_EUNCOMMITTED.rawValue)
    XCTAssertEqual(GitError.catching { throw GitError(code: .directory) }, GIT_EDIRECTORY.rawValue)
    XCTAssertEqual(
      GitError.catching { throw GitError(code: .mergeConflict) }, GIT_EMERGECONFLICT.rawValue)
    XCTAssertEqual(
      GitError.catching { throw GitError(code: .passthrough) }, GIT_PASSTHROUGH.rawValue)
    ...
    XCTAssertEqual(
      GitError.catching { throw GitError(code: .indexDirty) }, GIT_EINDEXDIRTY.rawValue)
  ...
  func testDescription() {
    ...
    XCTAssertEqual(
      GitError(domain: .noMemory, code: .notFound, message: "message").description,
      "[No Memory | Not Found] message")
    XCTAssertEqual(
      GitError(domain: .os, code: .exists, message: "message").description, "[OS | Exists] message")
    XCTAssertEqual(
      GitError(domain: .invalid, code: .ambiguous, message: "message").description,
      "[Invalid | Ambiguous] message")
    XCTAssertEqual(
      GitError(domain: .reference, code: .buffer, message: "message").description,
      "[Reference | Buffer] message")
    XCTAssertEqual(
      GitError(domain: .zlib, code: .bareRepository, message: "message").description,
      "[zlib | Bare Repository] message")
    XCTAssertEqual(
      GitError(domain: .repository, code: .unbornBranch, message: "message").description,
      "[Repository | Unborn Branch] message")
    XCTAssertEqual(
      GitError(domain: .config, code: .unmerged, message: "message").description,
      "[Config | Unmerged] message")
    XCTAssertEqual(
      GitError(domain: .regex, code: .nonFastForward, message: "message").description,
    ...
    XCTAssertEqual(
      GitError(domain: .tag, code: .auth, message: "message").description, "[Tag | Auth] message")
    XCTAssertEqual(
      GitError(domain: .tree, code: .certificate, message: "message").description,
      "[Tree | Certificate] message")
    XCTAssertEqual(
      GitError(domain: .indexer, code: .applied, message: "message").description,
      "[Indexer | Applied] message")
    XCTAssertEqual(
      GitError(domain: .ssl, code: .peel, message: "message").description, "[SSL | Peel] message")
    XCTAssertEqual(
      GitError(domain: .submodule, code: .endOfFile, message: "message").description,
      "[Submodule | End Of File] message")
    XCTAssertEqual(
      GitError(domain: .thread, code: .invalid, message: "message").description,
      "[Thread | Invalid] message")
    XCTAssertEqual(
      GitError(domain: .stash, code: .uncommitted, message: "message").description,
      "[Stash | Uncommitted] message")
    XCTAssertEqual(
      GitError(domain: .checkout, code: .directory, message: "message").description,
      "[Checkout | Directory] message")
    XCTAssertEqual(
      GitError(domain: .fetchHead, code: .mergeConflict, message: "message").description,
      "[Fetch HEAD | Merge Conflict] message")
    XCTAssertEqual(
      GitError(domain: .merge, code: .passthrough, message: "message").description,
      "[Merge | Passthrough] message")
    XCTAssertEqual(
      GitError(domain: .ssh, code: .iteratorOver, message: "message").description,
    ...
    XCTAssertEqual(
      GitError(domain: .callback, code: .indexDirty, message: "message").description,
      "[Callback | Index Dirty] message")
    XCTAssertEqual(
      GitError(domain: .cherrypick, code: .applyFail, message: "message").description,
      "[Cherrypick | Apply Fail] message")
    XCTAssertEqual(
      GitError(domain: .describe, code: .unknown).description, "[Describe | Unknown] General error."
    )
    XCTAssertEqual(
  ...
  func testCodeDetail() {
    ...
    XCTAssertEqual(
      GitError.Code.uncommitted.detail, "Uncommitted changes in index prevented operation.")
    XCTAssertEqual(GitError.Code.directory.detail, "The operation is not valid for a directory.")
    XCTAssertEqual(
      GitError.Code.mergeConflict.detail, "A merge conflict exists and cannot continue.")
    ...
    XCTAssertEqual(
      GitError.Code.indexDirty.detail, "Unsaved changes in the index would be overwritten.")
