...
extension Reference {
    ...
    init(_ reference: GitPointer) async throws {

        switch reference {

        case let reference where await reference.check(git_reference_is_branch):
            self = try await .branch(Branch(reference))

        case let reference where await reference.check(git_reference_is_note):
            self = try await .note(Note(reference))

        case let reference where await reference.check(git_reference_is_remote):
            self = try await .remoteBranch(RemoteBranch(reference))

        case let reference where await reference.check(git_reference_is_tag):
            self = try await .tag(Tag(reference))
...
extension Tagged where RawValue == Reference.ID {
    ...
    init(reference: GitPointer) async throws {
        let name = try await Unwrap(String(validatingUTF8: reference.get(git_reference_name)))
